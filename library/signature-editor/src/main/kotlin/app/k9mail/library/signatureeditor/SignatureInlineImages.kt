...
internal object SignatureInlineImages {
    ...
    fun encodeBytes(bytes: ByteArray, mimeHint: String?): String? {
        ...
        if (normalizedMime == "image/png" ||
            ...
            if (bytes.size <= MAX_ENCODED_BYTES && !exceedsMaxDimension(bytes)) {
                val mime = if (normalizedMime == "image/jpg") "image/jpeg" else normalizedMime
    ...
    private fun compressBitmap(bytes: ByteArray): String? {
        ...
        if (!hasAlpha && encoded.size > MAX_ENCODED_BYTES) {
            output.reset()
            // Re-decode from the already-scaled JPEG bytes is wasteful; shrink further.
            ...
            if (smaller !== retry) retry.recycle()
            smaller.compress(Bitmap.CompressFormat.JPEG, JPEG_QUALITY - 15, output)
            smaller.recycle()
            encoded = output.toByteArray()
        }

        if (encoded.size > MAX_ENCODED_BYTES * 2) {
